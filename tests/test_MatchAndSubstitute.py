...
class TestIsInstance(unittest.TestCase):
    def test_reflexivity_of_equals(self):
        self.assertEqual(
            is_instance(
                ex('M * P + M * Q == M * P + M * Q'), forall(A, ex('A == A')),
                set()), {A: ex('M * P + M * Q')})
...
class TestTryRule(unittest.TestCase):
    def test_doesnt_match(self):
        self.assertEqual(
            try_rule(
                ...
                ex('P + Q in B'), Direction.BACKWARD), set())

    def test_modus_tollens(self):
        self.assertEqual(
            try_rule(
                forall((P, Q), ex('((P ==> Q) and not Q) ==> not P')),
                ex('not B'), Direction.BACKWARD), {ex('(B ==> Q) and not Q')})
        # dummies: (P, Q).
    ...
    def test_modus_ponens(self):
        self.assertEqual(
            try_rule(
                ...
                ex('(A ==> B) and A'), Direction.FORWARD), {ex('B')})

    def test_definition_of_set(self):
        self.assertEqual(
            try_rule(
                forall(P, ex("P in B <==> P * M == M * P")), ex("P + Q in B"),
                Direction.BACKWARD), {ex('(P + Q) * M == M * (P + Q)')})

    def test_distribute_right(self):
        self.assertEqual(
            try_rule(
                forall((A, B, R), ex('(A + B) * R == A * R + B * R')),
                ex('(P + Q) * M == M * (P + Q)'), Direction.BACKWARD),
            {ex('P * M + Q * M == M * (P + Q)')})
    ...
    def test_distribute_left(self):
        self.assertEqual(
            try_rule(
                ...
                ex('P * M + Q * M == M * (P + Q)'), Direction.BACKWARD),
            {ex('P * M + Q * M == M * P + M * Q')})

    def test_known_property_of_P(self):
        self.assertEqual(
            try_rule(
                ex('P * M == M * P'), ex('P * M + Q * M == M * P + M * Q'),
                Direction.BACKWARD), {
                    ex('M * P + Q * M == M * P + M * Q'),
                    ex('P * M + Q * M == P * M + M * Q')
                })
    ...
    def test_cancel_both_sides(self):
        self.assertEqual(
            try_rule(
                ...
                ex('M * P + Q * M == M * P + M * Q'), Direction.BACKWARD), {
                    ex('Q * M == M * Q'),
    ...
    def test_no_match(self):
        self.assertEqual(
            try_rule(
                forall(P, ex('P + Q == Q + P')), ex('A and B'),
    ...
    def test_match_under_match(self):
        self.assertEqual(
            try_rule(
                forall((A, B), ex('A + B == B + A')), ex('X + Y + Z'),
                Direction.BACKWARD),  # backwards
            {ex('Z + (X + Y)'), ex('Y + X + Z')})
    ...
    def test_not_recursive(self):
        self.assertEqual(
            try_rule(
                forall((P, Q), ex('P and Q ==> P')), ex('not ( P and Q)'),
    ...
    def test_only_match_boolean(self):
        self.assertEqual(
            try_rule(
                forall((P, ), ex('P and P <==> P')), ex('A and B'),
                Direction.BACKWARD),  # backwards
            {
                ex('(A and A) and B'),
